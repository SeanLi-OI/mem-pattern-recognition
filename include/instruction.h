...
struct input_instr {
  ...
  uint64_t source_memory[NUM_INSTR_SOURCES] = {};            // input memorys

  uint64_t destination_memory_value[NUM_INSTR_DESTINATIONS] =
      {};                                                // output memory value
  uint64_t source_memory_value[NUM_INSTR_SOURCES] = {};  // input memory value
};
...
struct ooo_model_instr {
  ...
  std::vector<mpr::circular_buffer<ooo_model_instr>::iterator>
  ...
  uint64_t destination_memory[NUM_INSTR_DESTINATIONS_SPARC] =
      {};                                          // output memory
  uint64_t source_memory[NUM_INSTR_SOURCES] = {};  // input memory
  uint64_t destination_memory_value[NUM_INSTR_DESTINATIONS_SPARC] =
      {};                                                // output memory value
  uint64_t source_memory_value[NUM_INSTR_SOURCES] = {};  // input memory value
  ...
  ooo_model_instr(uint8_t cpu, input_instr instr) {
    ...
    std::copy(std::begin(instr.destination_memory_value),
              std::end(instr.destination_memory_value),
              std::begin(this->destination_memory_value));
    ...
    std::copy(std::begin(instr.source_memory_value),
              std::end(instr.source_memory_value),
              std::begin(this->source_memory_value));
