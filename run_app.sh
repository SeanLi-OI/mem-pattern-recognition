...
if [ "$RUN_MPR" = true ] ; then
    ...
    /home/lixiang/mem-pattern-recognition/bin/mpr ${mpr_trace_file} ${stat_file} ${pattern_file} 2>${bench_dir}/${app}/mpr_err.txt &
fi
...
if [ "$RUN_PARSER" = true ] ; then
    ...
    /home/lixiang/mem-pattern-recognition/result/parse ${miss_file} ${pattern_file} ${out_file} ${binary_file} 2>${bench_dir}/${app}/parse_err.txt
fi
