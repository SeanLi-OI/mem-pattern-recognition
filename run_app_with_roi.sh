...
trace_dir=${mpr_dir}/roi_test/trace
...
roi_file=${bench_dir}/${app}/roi.txt
...
if [ "$RUN_UNION_TRACE" = true ] ; then
    ...
    pin -t ${mpr_dir}/tracer/obj-intel64/union_tracer_with_roi.so -o ${champsim_trace_file} -m ${mpr_trace_file} -t ${trace_len} -s ${skip_len} -r ${roi_file} -- ${binary_file}  # <${app_input}
